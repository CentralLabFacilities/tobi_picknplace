...
public:
    ...
    Private(string service_name) :
            ...
            service_ptr_.reset(new ros::ServiceServer(nh.advertiseService("biron_posture", &Private::posture_callback, this)));
    }
    ...
    const string METHOD_PLACE = "placeOnSurface";
    const string METHOD_OPEN = "openGripper";
    const string METHOD_CLOSE = "closeGripper";
    ...
    boost::shared_ptr<Dictionary> graspObjectName(boost::shared_ptr<string> input, bool sim) {
        ...
        if (items.size() == 0) {
            ...
            grt = listener->requestGraspObject(items[0], "", sim); //surface0
        } else {
...
ServiceInterface::ServiceInterface(const string &serverScope, string service_name) :
