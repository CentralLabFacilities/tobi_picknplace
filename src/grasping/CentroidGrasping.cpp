 ...
 */
std::vector<moveit_msgs::Grasp> CentroidGrasping::generate_grasps_angle_trans(double x, double y,
        double z, double height) {

    ParamReader& params = ParamReader::getParamReader();

    ROS_INFO_NAMED("CentroidGrasping", "Generate grasps at %.3f,%.3f,%.3f (frame: %s) - h:%.3f", x, y, z, params.frameArm.c_str(), height);
    ...
    ROS_INFO_NAMED("CentroidGrasping", "Done generating %i grasps.", (int ) grasps.size());
 ...
 * x, y, z: center of grasp point (the point that should be between the finger tips of the gripper)
 */
std::vector<moveit_msgs::Grasp> CentroidGrasping::generate_grasps_angle_only(double x, double y,
        double z) {
...
std::vector<moveit_msgs::PlaceLocation> CentroidGrasping::generate_placeloc_angle_trans(double x, double y,
        double z) {

    ParamReader& params = ParamReader::getParamReader();

    ROS_INFO_NAMED("CentroidGrasping", "Generate place locations at %.3f,%.3f,%.3f (frame: %s)", x, y, z, params.frameArm.c_str());
    ...
    ROS_INFO_NAMED("CentroidGrasping", "Done generating %i place locations.", (int ) placelocs.size());

    return placelocs;

}
std::vector<moveit_msgs::PlaceLocation> CentroidGrasping::generate_placeloc_angle_only(double x, double y,
        double z) {
...
std::vector<moveit_msgs::PlaceLocation> CentroidGrasping::generate_place_locations(double x, double y,
        double z, tf::Quaternion targetOrientation) {
    ParamReader& params = ParamReader::getParamReader();

    ROS_INFO_NAMED("CentroidGrasping", "Generate place locations at %.3f,%.3f,%.3f (frame: %s) orientation: %.3f,%.3f,%.3f,%.3f,", x, y, z,
    ...
    ROS_INFO_NAMED("CentroidGrasping", "Done generating %i locations.", (int ) locations.size());
    return locations;
}
...
std::vector<moveit_msgs::PlaceLocation> CentroidGrasping::generate_place_locations(double x, double y,
    ...
    ROS_INFO_NAMED("CentroidGrasping", "Generate place locations at %.3f,%.3f,%.3f (frame: %s) orientation: %.3f,%.3f,%.3f,%.3f whd: %.3f,%.3f,%.3f", x, y, z,
...
//todo: h2r5
moveit_msgs::PlaceLocation CentroidGrasping::build_place_location(tf::Transform t) {
    ParamReader& params = ParamReader::getParamReader();
    static int j = 0;
    ROS_DEBUG_NAMED("CentroidGrasping", "Build place location at %.3f,%.3f,%.3f orientation: %.3f,%.3f,%.3f,%.3f", t.getOrigin().x(),t.getOrigin().y(), t.getOrigin().z(), t.getRotation().x(), t.getRotation().y(), t.getRotation().z(), t.getRotation().w());
